...
engine.setProperty(name="voice", value="english-us")
...
def page_scraper(flat_renting_url, pages_to_search):
    ...
    for page in range(pages_to_search):
        ...
        for link in links_to_offers:
            ...
            # max guests
            max_guests = soup.find(name="ol", class_="_194e2vt2")
...
def scraped_data_processing(titles, locations, dates, prices, images, links, max_guests_list):
    ...
    for i in range(len(titles)):
        ...
        date_in = f"{date_in_extract[2]}-{months[date_in_extract[0]]}-{date_in_extract[1][0]}"
        date_out = f"{date_out_extract[2]}-{months[date_out_extract[0]]}-{date_out_extract[1][0]}"
        regex = "\d+,\d+"
        regex_outcome = re.findall(regex, prices[i])
        # on some pages prices have different class, that's why "try/except" and zero price value
        # this can be fixed "by hand" with edit for price in "results/"
        try:
            thousands, hundreds = regex_outcome[0].split(",")
            price_after = thousands + hundreds
        except IndexError:
            price_after = 0
...
def voice_assistant():
    ...
    for i in form_fields:
        ...
        # TODO - ask if input was correct and if not get back to listening
        engine.say(form_fields[i]["sentence"] % variables[:-1])
        engine.runAndWait()
    ...
    # print(number_of_pages, date_in, date_out, adults, pets, real_estate_types, min_bedrooms, min_beds, min_bathrooms)
    flat_renting_url = f"https://www.airbnb.com/s/Denmark/" \
                       f"homes?tab_id=home_tab" \
                       f"&refinement_paths%5B%5D=%2Fhomes" \
                       f"&flexible_trip_dates[]=april&flexible_trip_dates[]=march" \
...
def command_interpreter(command, input_type):
    ...
    if input_type == "int":
        ...
        if output is None:
            try:
                ...
                engine.say("Could not understand. Please repeat.")
                engine.runAndWait()
    ...
    # consecutive digits (year)
        ...
        if day is not None and year is not None:
            ...
            engine.say("That's not a right date. Please repeat.")
            engine.runAndWait()
...
def speech_recognition():
    ...
    while run:
        with sr.Microphone() as mic:
            ...
            voice = listener.listen(mic, timeout=20)
        try:
            print("Recognizing...")
